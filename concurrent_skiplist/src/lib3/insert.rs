...
pub(super) fn insert<K,V>(lanes: &[AtomicPtr<Node<K,V>>], elem: (K,V), max_height: &AtomicU8)
                            -> Option<V>
    where K: Ord
{
    ...
    let mut elem: ManuallyDrop<(K,V)> = ManuallyDrop::new(elem);
    let mut elem_ptr: NonNull<(K,V)> = NonNull::from(&*elem);
            ...
            'down: for atomic_ptr in lanes {
                ...
                match ptr {
                    ...
                    Some(ptr)   => unsafe {
                        ...
                        let (kref,vref) = elem_ptr.as_ref();

                        match kref.cmp(&node.inner.elem.0) {
                            ...
                            Equal   => match &mut new_node {
                                ...
                                None            => {
                                    let aa=ManuallyDrop::take(&mut elem);
                                    return Some(aa.1);
                                }
                            }
